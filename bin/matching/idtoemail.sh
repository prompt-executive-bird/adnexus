...
##
## idtoemail.sh - Retrieves an email from the session database matching a given
##                ID
#
#  Usage: idtoemail.sh <id>
#  Returns: (0) Email of participant (stdout)
...
# Database query
#
# Pull the email information from the database, alongside the COUNT of such
# email addresses. The COUNT should be 1, indicating an error otherwise.
#
...
QUERY_RESULT="$(sqlite3 "$DATABASE" "
    SELECT COUNT(email),email FROM contact
    WHERE id = \"$PERSON\";")"
QUERY_COUNT="$(echo $QUERY_RESULT | sed -e 's/^\(.*\)|\(.*\)$/\1/')"
QUERY_EMAIL="$(echo $QUERY_RESULT | sed -e 's/^\(.*\)|\(.*\)$/\2/')"
